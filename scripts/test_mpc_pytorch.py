...
def main(cfg):
    """
    mpc.pytorch 스타일 differentiable MPC(iLQR) 환경이 step을 돌 수 있는지 확인하는 스모크 테스트.

    예)
      python scripts/test_mpc_pytorch.py task=Hover_MPC_PyTorch headless=false enable_livestream=false env.num_envs=1
    """
    ...
    print("[test_mpc_pytorch] seed:", seed)
    env.set_seed(seed)
