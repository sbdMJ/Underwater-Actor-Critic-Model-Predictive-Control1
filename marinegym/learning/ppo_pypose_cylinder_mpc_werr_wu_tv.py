...
class PPOPyposeCylinderMPCWErrWUTVConfig:
    ...
    # When orbit error is large, relax extra input-penalty so MPC can respond aggressively to disturbances.
    R_err_relax_scale: float = 0.5
...
class _MPCMeanActorOrbitErrTV(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        relax_scale = max(float(getattr(self.cfg, "R_err_relax_scale", 0.5)), 1e-6)
        # Larger orbit error -> smaller additive R (less input penalty, more corrective authority).
        r_gate = torch.exp(-orbit_err / relax_scale)
        R_add = ru_ref * float(self.cfg.R_min_coeff) * r_gate
        R = R_base + R_add
