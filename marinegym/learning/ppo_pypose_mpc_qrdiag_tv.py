...
@dataclass
class PPOPyposeMPCQRDiagTVConfig:
    """
    PPO(actor-critic) + PyPose differentiable MPC(iLQR) as the last layer of the actor.

    - A neural cost-map maps observation -> time-varying diag(Q_t), diag(R_t) for t in [0..H-1].
    - The actor samples actions from N(loc=MPC(u0), scale=exp(log_std)).
    """

    name: str = "ppo_pypose_mpc_qrdiag_tv"
    train_every: int = 32
    ppo_epochs: int = 4
    ...
    # Learnable exploration std (log-space), closer to typical PPO + the ACMPC paper.
...
cs.store("ppo_pypose_mpc_qrdiag_tv", node=PPOPyposeMPCQRDiagTVConfig, group="algo")
cs.store(
    "ppo_pypose_mpc_qrdiag_tv_priv",
    node=PPOPyposeMPCQRDiagTVConfig(priv_actor=True, priv_critic=True),
    group="algo",
)
cs.store(
    "ppo_pypose_mpc_qrdiag_tv_priv_critic",
    node=PPOPyposeMPCQRDiagTVConfig(priv_critic=True),
    group="algo",
)
...
def _load_uav_params(cfg: PPOPyposeMPCQRDiagTVConfig) -> dict:
...
class _NeuralDiagCostMapHorizon(nn.Module):
    def __init__(self, cfg: PPOPyposeMPCQRDiagTVConfig, *, horizon: int, nx: int, nu: int):
        super().__init__()
        self.cfg = cfg
...
class _MPCMeanActorTV(nn.Module):
    def __init__(
        self,
        cfg: PPOPyposeMPCQRDiagTVConfig,
        *,
        nu: int,
        device,
    ):
        ...
        self.cost_map = _NeuralDiagCostMapHorizon(cfg, horizon=int(cfg.mpc_horizon), nx=self.nx, nu=self.nu).to(device)
...
class PPOPyposeMPCQRDiagTVPolicy(TensorDictModuleBase):
    def __init__(
        self,
        cfg: PPOPyposeMPCQRDiagTVConfig,
        ...
        mpc_mean = _MPCMeanActorTV(cfg, nu=self.action_dim, device=device)

        if self.cfg.priv_actor:
            raise NotImplementedError(
                "priv_actor is disabled for ppo_pypose_mpc_qrdiag_tv to keep the actor learnables limited to the cost map and log_std."
            )
